...
// 1) SSR: fetch initial session to hydrate SessionContextProvider in _app.tsx
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  const supabaseServer = createServerSupabaseClient({ req: ctx.req, res: ctx.res })
  const {
    data: { session },
...
const AvatarProfile: React.FC = () => {
  ...
  // 3) Dynamic userName state with default
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      // Raw fallback if context not ready
      supabaseClient.auth.getUser().then(({ data }) => {
        const u = data.user
        if (u) {
          const name =
            u.user_metadata?.avatarName ||
  ...
  return (
    <div className="container mx-auto px-4 py-8 pt-24 space-y-8">
      ...
      {/* Tabs Section */}
      <Tabs defaultValue="appearance" className="space-y-6 mt-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card className="glass-card border-accent/20">
              ...
              <CardContent className="space-y-4">
                {['Hair Style', 'Facial Features', 'Body Type', 'Clothing', 'Accessories'].map(
                  (opt, i) => (
                    <div
                      key={i}
